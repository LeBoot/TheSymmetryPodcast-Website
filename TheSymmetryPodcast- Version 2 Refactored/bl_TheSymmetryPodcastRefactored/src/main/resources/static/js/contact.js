...
//CONFIGURE DROPDOWN ----------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------
function configureDropdown(accountNum, regionId) {
	
	$("#regionInput").html(
		`<option value="Default" disabled selected hidden>Where are you listening from?*</option>`
	);
	
	configureDropdownAjaxCall(accountNum, regionId);
	
}

function configureDropdownAjaxCall(accountNumPassed, regionIdPassed) {
	...
	$.ajax({
        ...
        success: function(regionArray) {
			...
			if (accountNumPassed == -1) {
				...
				$.each(regionArray, function(index, region) {
					if (region.regionid == regionIdPassed) {
						var regionsInput = '<option selected value="' + region.regionid + '">' + region.regionName + '</option>'
					} else {
						var regionsInput = '<option value="' + region.regionid + '">' + region.regionName + '</option>'
					}
					regionsDiv.append(regionsInput);
...
//FILL IN INFORMATION FOR LOGGED-IN USERS -------------------------------------------------------
//-----------------------------------------------------------------------------------------------
...
function fillInfo(accountNum) {
	
	$.ajax({
        type: 'GET',
        url: 'http://localhost:8080/account/get/' + accountNum,
        success: function(account) {	
			$("#nameInput").val(account.firstName + " " + account.lastName);
			$("#emailInput").val(account.email);
			configureDropdown(accountNum, account.region.regionid);
        },
        error: function (jqXHR, textStatus, errorThrown) {
        }
    });
